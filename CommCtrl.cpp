...
#define COMM_OKPOS_HEADER	0x4d //OKPOS 여권스캐너 체크 헤더 추가
...
INT AutoDetect(HWND hWnd, INT iBaudRate , int nType)
{
	...
	CString strTemp = "";
	strTemp.Format("AutoDetect >> type:%d",nType);
	//AfxMessageBox(strTemp);

	for(iPort=1; iPort<30; iPort++)	{
		...
		}else if (nType == 3)
		{
			...
			bFound = CheckPort_OKPOS(hWnd, iPort, iBaudRate );
		}else
		{
			bFound = CheckPort(hWnd, iPort, iBaudRate );
		}
